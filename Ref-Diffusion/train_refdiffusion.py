...
import open_clip
from ref_net import Image_adapter
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
               ...
               # Predict the noise residual
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    # Debugging message
                    print(f"Unknown prediction type: {noise_scheduler.config.prediction_type}")
                    raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")

                # Ensure target has been assigned a value
                if target is None:
                    raise ValueError("Target is not initialized. Check the noise scheduler and prediction type logic.")
            ...
            # Content Loss (L1 or L2 Loss) on feature maps (from the last layer, for example)
            SCL_loss = F.mse_loss(model_pred_features[-1], target_features[-1], reduction="mean")

            # Style Loss: Compare Gram matrices of the feature maps
            style_loss = 0
            for mp_feat, tgt_feat in zip(model_pred_features, target_features):
                style_loss += F.mse_loss(gram_matrix(mp_feat), gram_matrix(tgt_feat))


            FDL_loss = 0
            for mp_feat, tgt_feat in zip(model_pred_features, target_features):
                FDL_loss += 1 - F.cosine_similarity(mp_feat.flatten(), tgt_feat.flatten(), dim=0).mean()
            # Slice the extra channel from text_pred


            # Now calculate the loss with matching shapes
            # loss_aux1 = F.mse_loss(text_pred.float(), target.float(), reduction="mean")

            # Add the new losses to your existing loss
            loss = F.mse_loss(model_pred, target, reduction="mean")

            zero_channel = torch.zeros_like(target[:, :1, :, :])
            target = torch.cat([target, zero_channel], dim=1)


            def compute_infonce_loss(anchor, positive, negatives, temperature=0.07):
                """
                Compute the InfoNCE loss between anchor and positive samples and multiple negatives.
                Handle case where negatives are None (i.e., no negatives in the batch).
                """
                # Normalize the anchor and positive features
                anchor = F.normalize(anchor, dim=-1)
                positive = F.normalize(positive, dim=-1)

                # Check if negatives exist (i.e., are not None)
                if negatives is None:
                    # No negatives, return a loss of 0
                    return torch.tensor(0.0, device=anchor.device)

                # Normalize negatives if they exist
                ...
                # Define anchor (from model_pred) and positive (from target) features
                # Here we use the second-to-last layer features for InfoNCE loss

            model_pred = model_pred[:, :3, :, :]  # Keep only the first 3 channels
            target = target[:, :3, :, :]  # Keep only the first 3 channels

            # Extract VGG features for both model_pred (generated image) and target (reference image)
            model_pred_features = vgg_feature_extractor(model_pred)  # Features from model_pred
            target_features = vgg_feature_extractor(target)  # Features from target
            ...
            # Identity Loss: Cosine similarity between flattened features
            FDL_loss = 0
            for mp_feat, tgt_feat in zip(model_pred_features, target_features):
               FDL_loss += 1 - F.cosine_similarity(mp_feat.flatten(), tgt_feat.flatten(), dim=0).mean()

            # Define anchor (from model_pred) and positive (from target) features for InfoNCE loss
            # Using features from the second-to-last layer
            anchor_features = model_pred_features[-2]
            positive_features = target_features[-2]

            # Check if there are enough samples for negative examples
            if target_features[-2].size(0) > 1:
                # Create negative samples from the batch (if applicable)
                negatives = torch.cat(
                    [target_features[-2][i].unsqueeze(0) for i in range(1, target_features[-2].size(0))], dim=0)
            else:
                negatives = None  # No negatives available

            # Compute InfoNCE loss
            CIL_loss = compute_infonce_loss(anchor_features, positive_features, negatives)

            # Compute the base loss between model_pred and target
            loss = F.mse_loss(model_pred, target, reduction="mean")

            # Now compute the total loss
            total_loss = (loss + 0.001 * SCL_loss+ 0.01 * FDL_loss+ 0.1 * CIL_loss)

            # Continue with backpropagation and optimization as before
